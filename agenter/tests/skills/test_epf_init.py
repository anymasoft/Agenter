...
"""Tests for the `epf-init` skill.

Auto-generated stub. Fill in fixtures and assertions following
the patterns in `tests/skills/test_subsystem_edit.py`.

Source: `.claude/skills/epf-init/SKILL.md`
Entry kind: ps1
Entry script: `.claude/skills/epf-init/scripts/init.ps1`
"""
...
def test_skill_script_exists(skills_dir: Path):
    """A PowerShell entry point named `init.ps1` should exist."""
    script = skills_dir / "epf-init" / "scripts" / "init.ps1"
...
@pytest.mark.skip(reason="auto-stub: implement edge case for 'Файл уже существует: $rootFile'")
def test_edge_rootfile(skills_dir: Path, fixture_factory):
    """Trigger PS1 error: 'Файл уже существует: $rootFile'."""
