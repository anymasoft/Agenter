...
"""Tests for the `template-remove` skill.

Auto-generated stub. Fill in fixtures and assertions following
the patterns in `tests/skills/test_subsystem_edit.py`.

Source: `.claude/skills/template-remove/SKILL.md`
Entry kind: ps1
Entry script: `.claude/skills/template-remove/scripts/remove-template.ps1`
"""
...
def test_skill_script_exists(skills_dir: Path):
    """A PowerShell entry point named `remove-template.ps1` should exist."""
    script = skills_dir / "template-remove" / "scripts" / "remove-template.ps1"
...
@pytest.mark.skip(reason="auto-stub: implement edge case for 'Метаданные макета не найдены: $templateMetaPath'")
def test_edge_templatemetapath(skills_dir: Path, fixture_factory):
    """Trigger PS1 error: 'Метаданные макета не найдены: $templateMetaPath'."""
    raise NotImplementedError
