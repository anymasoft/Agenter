...
class DesktopAPI:
    ...
    def sync_base(self) -> dict:
        """Фаза 2 / Шаг 2.3 — «Синхронизировать с базой» (Уровень 1).

        Ручная страховка: принудительный db_dump (выгрузка расширения/SCHEME) +
        переиндексация BSL Atlas. После этого индекс/SCHEME отражают РЕАЛЬНОЕ
        состояние базы — именно из них guard читает имена объектов, не из
        записанной памяти (принцип Р4). Используется, если авто-сверка (Шаг 2.2)
        выключена/не сработала или базу правили вручную.

        Семантически = full indexing pipeline (`_run_indexing`: scheme + ext +
        atlas reindex), который и есть db_dump + reindex против живой 1С."""
        return self.reindex()
