...
function Do-AddContent([string[]]$items) {
	...
	if (-not $contentEl) {
		# Auto-create <Content> when missing.
		# Borrowed subsystems from cfe_borrow start without it;
		# we create it on first add-content instead of erroring out.
		$contentEl = $script:xmlDoc.CreateElement("Content", $script:mdNs)
		$propsChildIndent = Get-ChildIndent $script:propsEl
		Insert-BeforeElement $script:propsEl $contentEl $null $propsChildIndent
		Info "Created missing <Content> element"
	}
...
function Do-RemoveContent([string[]]$items) {
	...
	if (-not $contentEl) {
		# Idempotent: removing from an absent collection is a no-op success.
		Warn "No <Content> element - nothing to remove (idempotent no-op)"
		return
	}
...
function Do-AddChild([string]$childName) {
	if (-not $script:childObjsEl) {
		# Auto-create <ChildObjects> when missing (borrowed-empty subsystem case).
		$script:childObjsEl = $script:xmlDoc.CreateElement("ChildObjects", $script:mdNs)
		$subChildIndent = Get-ChildIndent $script:sub
		Insert-BeforeElement $script:sub $script:childObjsEl $null $subChildIndent
		Info "Created missing <ChildObjects> element"
	}
...
function Do-RemoveChild([string]$childName) {
	if (-not $script:childObjsEl) {
		# Idempotent: removing from an absent collection is a no-op success.
		Warn "No <ChildObjects> element - nothing to remove (idempotent no-op)"
		return
	}
